                                        ...
                                        <div class="card-body">
                                                ...
                                                <div class="col-md-4">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" name="orderCode" id="orderCode"
                                                            ...
                                                            placeholder="Order Code" required>
                                                    </div>
                                                </div>
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                            ...
                                            <div id="supplierInfo" class="alert alert-info py-2 d-none">
                                                <%-- <i class="fas fa-info-circle mr-1"></i>--%>
                                                    Supplier selected. You can add products below.
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div id="emptyMsg" class="text-center text-muted py-4">
                                                <%-- <i class="fas fa-arrow-up fa-2x mb-2 d-block"></i>--%>
                                                    Select a Supplier first, then add products
                                            </div>
                                            ...
                                            <div
                                                ...
                                                <span>
                                                    <%-- <i class="fas fa-calculator mr-2"></i> --%>
                                                        Grand Total:</span>
                                                <span id="grandTotal" class="font-weight-bold">0 VND</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header" style="background:#28a745; color:#fff;">
                                                <h5 class="modal-title"><i class="fas fa-box mr-2"></i>Create New
                                                    Product</h5>
                                                <button type="button" class="close text-white"
                                                    data-dismiss="modal">&times;</button>
                                            </div>
            ...
            <script>
                ...
                function addProductRow(productToAdd) {
                    ...
                    const html =
                        ...
                        '</select>' +
                        '<div class="input-group-append">' +
                        '<button type="button" class="btn btn-outline-success btn-new-product" data-row="' + idx + '" title="Create new product"><i class="fas fa-plus"></i></button>' +
                        '</div>' +
                        '</div>' +
                        '</div>' +
                        '<div class="col-md-3">' +
                        '<label class="small font-weight-bold">Product Variant (Color / Lot / Serial) *</label>' +
                        '<select name="productDetailId[]" id="pdId_' + idx + '" class="form-control" required>' +
                        '<option value="">-- Select Variant --</option>' +
                        '</select>' +
                        '</div>' +
                        '<div class="col-md-1">' +
                        '<label class="small font-weight-bold" title="Quantity">Qty *</label>' +
                        '<input type="number" name="quantity[]" id="qty_' + idx + '" class="form-control qty-input" data-row="' + idx + '" value="1" min="1" required>' +
                        '</div>' +
                        '<div class="col-md-2">' +
                        '<label class="small font-weight-bold">Unit Price (VND) *</label>' +
                    ...
                    document.getElementById('prodSel_' + idx).addEventListener('change', function () {
                        ...
                        const pdSel = document.getElementById('pdId_' + idx);
                        ...
                        if (prod && prod.details && prod.details.length > 0) {
                            ...
                            prod.details.forEach(function (d) {
                                ...
                                let extra = [];
                                if (d.lotNumber && d.lotNumber !== 'N/A') extra.push('Lot: ' + d.lotNumber);
                                if (d.serial && d.serial !== 'N/A') extra.push('Ser: ' + d.serial);
                            ...
                            pdSel.innerHTML = opts;
                        } else {
                            pdSel.innerHTML = '<option value="">-- No variants --</option>';
                        }
                        priceInput.value = 0;
                        calcRow(idx);
                    });

                    // BẮT SỰ KIỆN KHI ĐỔI MÀU/VARIANT -> TỰ AUTO FILL ĐƠN GIÁ
                    document.getElementById('pdId_' + idx).addEventListener('change', function () {
                        const opt = this.options[this.selectedIndex];
                        const priceInput = document.getElementById('price_' + idx);
                        if (opt && opt.dataset.price) {
                            priceInput.value = opt.dataset.price;
                        } else {
                            // If no price set on variant, we can keep the manually entered price or default to 0
                            // But user wants to manage price in PO, so 0 is fine if variant price is 0
                ...
                function calcGrandTotal() {
                    ...
                    document.getElementById('grandTotal').textContent = numberFormat(total) + ' VND';
                }
                ...
                document.getElementById('btnSaveSupplier').addEventListener('click', function () {
                    ...
                    if (!name) {
                        showMsg('supplierFormMsg', 'danger', 'Supplier name cannot be empty!');
                        return;
                    }
                    ...
                    fetch(ctx + '/quick-add-supplier', {
                        ...
                        .then(function (data) {
                            if (data.success) {
                                ...
                                showMsg('supplierFormMsg', 'success', 'Supplier "' + data.supplierName + '" has been created!');
                                setTimeout(function () { $('#modalAddSupplier').modal('hide'); hideMsg('supplierFormMsg'); }, 1200);
                            } else {
                                showMsg('supplierFormMsg', 'danger', data.message || 'Unknown error');
                            }
                        })
                        .catch(function () { showMsg('supplierFormMsg', 'danger', 'Server connection error'); })
                        .finally(function () {
                            btn.disabled = false;
                ...
                document.getElementById('btnSaveProduct').addEventListener('click', function () {
                    ...
                    if (!name || !code) {
                        showMsg('productFormMsg', 'danger', 'Product Name and Code cannot be empty!');
                        return;
                    }
                    ...
                    fetch(ctx + '/quick-add-product', {
                        ...
                        .then(function (data) {
                            if (data.success) {
                                ...
                                showMsg('productFormMsg', 'success', 'Product "' + data.productName + '" has been created!');
                                setTimeout(function () { $('#modalAddProduct').modal('hide'); hideMsg('productFormMsg'); }, 1200);
                            } else {
                                showMsg('productFormMsg', 'danger', data.message || 'Unknown error');
                            }
                        })
                        .catch(function () { showMsg('productFormMsg', 'danger', 'Server connection error'); })
                        .finally(function () {
                            btn.disabled = false;
                ...
                document.getElementById('poForm').addEventListener('submit', function (e) {
                    ...
                    rows.forEach(function (row) {
                        ...
                        const variantSel = row.querySelector('select[name="productDetailId[]"]');
                        if (!prodSel || !prodSel.value || !variantSel || !variantSel.value) valid = false;
                    });
                    if (!valid) {
                        e.preventDefault();
                        alert('Please select both a product and a specific variant (Lot/Serial/Color) for all rows!');
                    }
                });
