                        ...
                        <div class="modal-dialog modal-dialog-centered" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title font-weight-bold"><i class="ri-truck-line mr-2"></i>New
                                        Supplier
                                    </h5>
                        ...
                        <div class="modal-dialog modal-dialog-centered" role="document">
                            <div class="modal-content">
                                <div class="modal-header" style="border-bottom: 2px solid #f1f5f9;">
                                    <h5 class="modal-title font-weight-bold"><i class="ri-box-3-line mr-2"></i>New
                                        Product
                                    </h5>
                                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                                </div>
                                <div class="modal-body">
                                    ...
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="text" id="prod_supplierName" class="form-control bg-light"
                                            readonly>
                    ...
                    <script>
                        ...
                        const allProducts = [
                            <c:forEach var="p" items="${productList}">
                                {id: ${p.id}, name: "${fn:escapeXml(p.name)}", code: "${fn:escapeXml(p.code)}", price: ${not empty p.price ? p.price : 0}, color: "${fn:escapeXml(p.color)}" },
                            </c:forEach>
                        ];
                        ...
                        function buildProductOptions() {
                            ...
                            if (currentSupplierProducts && currentSupplierProducts.length > 0) {
                                currentSupplierProducts.forEach(function (p) {
                                    // Thêm data-color và data-price vào option
                                    let prodColor = p.color ? p.color : 'Chưa có màu';
                                    let displayPrice = numberFormat(p.price) + ' đ';
                                    opts += '<option value="' + p.id + '" data-price="' + p.price + '" data-color="' + prodColor + '" data-detail-id="' + p.detailId + '">' + p.name + ' [' + p.code + '] - ' + prodColor + ' (' + displayPrice + ')</option>';
                                });
                            } else {
                                if (selectedSupplierId) {
