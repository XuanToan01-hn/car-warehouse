                            ...
                            <div class="page-header d-flex justify-content-between align-items-center">
                                <div>
                                    <a href="javascript:history.back()"
                                        class="text-primary font-weight-bold mb-2 d-inline-block">
                                ...
                                <div id="statusArea">
                                    <c:choose>
                                        <c:when test="${t.status == 0}"><span
                                                class="status-badge badge-warning">Pending</span></c:when>
                                        <c:when test="${t.status == 1}"><span
                                                class="status-badge badge-info">Approved</span></c:when>
                                        <c:when test="${t.status == 2}"><span
                                                class="status-badge badge-primary">In-Transit</span></c:when>
                                        <c:when test="${t.status == 3}"><span
                                                class="status-badge badge-success">Completed</span></c:when>
                                        <c:otherwise><span class="status-badge badge-secondary">Cancelled</span>
                                        </c:otherwise>
                                    </c:choose>
                                </div>
                            </div>
                                    ...
                                    <div class="row">
                                        <div class="col-lg-6">
                                            <div class="product-banner mb-4"
                                                style="background: none; border: 1px solid #e2e8f0; padding: 0; overflow: hidden;">
                                                <div class="bg-primary text-white px-3 py-2">
                                                    <span class="detail-label text-white mb-0"
                                                        style="font-size: 0.7rem;">Car / Product List</span>
                                                </div>
                                                    ...
                                                    <table class="table table-hover mb-0" style="font-size: 0.9rem;">
                                                        <thead class="bg-light">
                                                            <tr>
                                                                ...
                                                                <th class="py-2 text-center" style="width: 80px;">Qty
                                                                </th>
                                                            </tr>
                                                        </thead>
                                                ...
                                                <div id="productPagination"
                                                    ...
                                                    <small class="text-muted" id="pageInfo">Items 1-5 of 10</small>
                                                    <div class="btn-group btn-group-sm">
                                                        <button type="button" class="btn btn-outline-primary"
                                                            id="prevBtn" onclick="changePage(-1)"><i
                                                                class="ri-arrow-left-s-line"></i></button>
                                                        <div id="pageNumbers" class="btn-group btn-group-sm"></div>
                                                        <button type="button" class="btn btn-outline-primary"
                                                            id="nextBtn" onclick="changePage(1)"><i
                                                                class="ri-arrow-right-s-line"></i></button>
                                                    </div>
                                                </div>
                                            </div>
                                            <script>
                                                // Data from Server (evaluated at JSP compile time)
                                                const allItems = [
                                                    <c:forEach var="item" items="${productList}" varStatus="st">
                                                        {name: "${item.name}", qty: ${item.qty} }${st.last ? '' : ','}
                                                    </c:forEach>
                                                ];
                                                ...
                                                function renderTable() {
                                                    ...
                                                    if (allItems.length > pageSize) {
                                                        ...
                                                        document.getElementById('pageInfo').textContent = 'Items ' + (start + 1) + '-' + end + ' of ' + allItems.length;
                                                        document.getElementById('prevBtn').disabled = (currentPage === 1);
                                                        document.getElementById('nextBtn').disabled = (currentPage === totalPages);

                                                        let buttons = '';
                                                        for (let i = 1; i <= totalPages; i++) {
                                                            buttons += '<button type="button" class="btn ' + (i === currentPage ? 'btn-primary' : 'btn-outline-primary') + '" onclick="goToPage(' + i + ')">' + i + '</button>';
                                                        }
                                                        document.getElementById('pageNumbers').innerHTML = buttons;
                                                    } else {
                                            ...
                                            <div class="info-section">
                                                ...
                                                <div class="mt-2 p-3 border rounded bg-white text-muted"
                                                ...
                                                <div class="d-flex flex-column gap-4">
                                                    <div class="route-node">
                                                        ...
                                                        <div style="line-height: 1.4;">
                                                            <small
                                                                class="text-muted d-block text-uppercase font-weight-bold"
                                                                style="font-size: 0.7rem;">Source Location</small>
                                                            <span class="detail-value">${t.fromLocationName}</span>
                                                            <small
                                                                class="text-muted d-block">${t.fromWarehouseName}</small>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="route-node">
                                                        ...
                                                        <div style="line-height: 1.4;">
                                                            <small
                                                                class="text-muted d-block text-uppercase font-weight-bold"
                                                                style="font-size: 0.7rem;">Destination Location</small>
                                                            <span class="detail-value">${t.toLocationName}</span>
                                                            <small
                                                                class="text-muted d-block">${t.toWarehouseName}</small>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <c:choose>
                                            <c:when test="${t.status == 0}">
                                                ...
                                                <form action="internal-transfer" method="POST" class="mr-3">
                                                    ...
                                                    <button type="submit"
                                                        class="btn btn-outline-danger px-4 font-weight-bold rounded-pill"
                                                        onclick="return confirm('Cancel this request?')">Cancel
                                                        Request</button>
                                                </form>
                                                <form action="internal-transfer" method="POST">
                                                    ...
                                                    <input type="hidden" name="transferId" value="${t.id}">
                                                    <button type="submit"
                                                        class="btn btn-primary px-5 font-weight-bold rounded-pill shadow-sm"
                                                        onclick="return confirm('Approve this request?')">Approve
                                                        Request</button>
                                                </form>
                                            </c:when>
                                            <c:when test="${t.status == 1}">
                                                <form action="warehouse-transfer" method="POST">
                                                    ...
                                                    <input type="hidden" name="transferId" value="${t.id}">
                                                    <button type="submit"
                                                        class="btn btn-warning px-5 font-weight-bold rounded-pill shadow-sm"
                                                        onclick="return confirm('Confirm Export?')">Confirm
                                                        Export</button>
                                                </form>
                                            </c:when>
                                            <c:when test="${t.status == 2}">
                                                <form action="warehouse-transfer" method="POST">
                                                    ...
                                                    <input type="hidden" name="transferId" value="${t.id}">
                                                    <button type="submit"
                                                        class="btn btn-success px-5 font-weight-bold rounded-pill shadow-sm"
                                                        onclick="return confirm('Confirm Import?')">Confirm
                                                        Import</button>
                                                </form>
                                            </c:when>
                                        </c:choose>
                                        <button class="btn btn-secondary px-4 ml-3 rounded-pill"
