...
(function (jQuery) {
    ...
    /*---------------------------------------------------------------------
     Datatables
     -----------------------------------------------------------------------*/
    var originalOrder = []; // lưu thứ tự gốc ban đầu
    ...
    var table = $('.data-tables').DataTable({
        ...
        columnDefs: [
            ...
            {orderable: true, targets: [0, 2]} // Sắp xếp được cột Product và Export Price
        ],
        pageLength: 10,
        lengthChange: true,
        initComplete: function () {
            ...
            // Ghi nhớ lại dữ liệu gốc (thứ tự gốc)
            originalOrder = api.rows().indexes().toArray();

            // ==== Lọc theo hãng (brand-filter) ====
            var brandIndex = 4;
            var uniqueBrands = new Set();
            api.column(brandIndex).data().each(function (d) {
                uniqueBrands.add(d);
            });

            Array.from(uniqueBrands).sort().forEach(function (brand) {
                $('#brand-filter').append('<option value="' + brand + '">' + brand + '</option>');
            });

            $('#brand-filter').on('change', function () {
                var val = $.fn.dataTable.util.escapeRegex($(this).val());
                api.column(brandIndex).search(val ? '^' + val + '$' : '', true, false).draw();
            });

            // ==== Sắp xếp theo giá (price-sort) ====
            $('#price-sort').on('change', function () {
                var sortOrder = $(this).val();
                var priceIndex = 2;

                if (sortOrder === "") {
                    api.order([]).draw(); // Reset lại
                } else {
                    api.order([priceIndex, sortOrder]).draw();
                }
            });

            // ==== Giới hạn tìm kiếm chỉ theo cột "Product" (index 0) ====
            $('.dataTables_filter input').off().on('keyup', function () {
                api.column(0).search(this.value).draw(); // Chỉ tìm trong cột Product
            });
        }
    });
